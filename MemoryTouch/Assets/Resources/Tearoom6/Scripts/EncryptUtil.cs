...
public class EncryptUtil
{
    ...
    {
        return Encrypt(ObjectToJson(value), password);
    }
    ...
    public static string Encrypt(string sourceString, string password)
    {
        ...
        // 暗号化
        byte[] encryptedCode = Encrypt(strBytes, password);
        // バイト型配列を文字列に変換して返す
        return System.Convert.ToBase64String(encryptedCode);
    }
    ...
    {
        return JsonToObject<T>(Decrypt(sourceString, password));
    }

    /// <summary>
    /// Rijndael暗号化された文字列を復号します。
    /// </summary>
    /// <param name="sourceString">暗号化された文字列</param>
    /// <param name="password">暗号化に使用したパスワード</param>
    ...
    public static string Decrypt(string sourceString, string password)
    {
        ...
        // 復号
        byte[] decriptedCode = Decrypt(strBytes, password);
        // バイト型配列を文字列に戻して返す
        return System.Text.Encoding.UTF8.GetString(decriptedCode);
    }

    /// <summary>
    /// Rijndael暗号化されたバイト列を復号します。
    /// </summary>
    /// <param name="sourceBytes">Source bytes.</param>
    /// <param name="password">Password.</param>
    public static byte[] Decrypt(byte[] sourceBytes, string password)
    {
        // RijndaelManagedオブジェクトを作成
        System.Security.Cryptography.RijndaelManaged rijndael =
            new System.Security.Cryptography.RijndaelManaged();
